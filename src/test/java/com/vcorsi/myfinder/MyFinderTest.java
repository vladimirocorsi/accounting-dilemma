...
public class MyFinderTest {
	...
	public void testTrivialSolution() {
		...
		final Finder finder1 = FinderFactory.finder(values, BigDecimal
				.valueOf(70));

		final List<BigDecimal> l = Arrays.asList(finder1.find());
		assertFalse(l.isEmpty());

		final Finder finder2 = FinderFactory.finder(values, BigDecimal
				.valueOf(72));
	...
	public void testDecimalSolution() {
		...
		final Finder finder = FinderFactory.finder(values, new BigDecimal(
	...
	public void testWithNoInputValues() {
		final BigDecimal[] values = new BigDecimal[] {};
		try {
			FinderFactory.finder(values, new BigDecimal(
					new BigInteger("6070"), 2));
			fail();
	...
	public void testZeroSumS() {
		...
		final Finder finder = FinderFactory.finder(values, BigDecimal.ZERO);
		final List<BigDecimal> l = Arrays.asList(finder.find());
